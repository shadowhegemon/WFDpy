...
def lookup_callsign_qrz(callsign):
    """Lookup callsign information from QRZ.com (requires session key)"""
    try:
        # QRZ.com requires login session - this is a placeholder implementation
        # In production, you would need to implement proper QRZ XML API authentication
        # For now, we'll return basic mock data structure
        return {
            'source': 'QRZ.com',
            ...
            'country': None,
            'state': None,
            'error': 'QRZ.com API requires authentication - not implemented in demo'
        }
    except Exception as e:
        return {'error': f'QRZ lookup failed: {str(e)}'}

def lookup_callsign_hamqth(callsign):
    """Lookup callsign information from HamQTH.com (free, no API key required)"""
    try:
        # HamQTH.com provides free XML API without authentication for basic info
        url = f"https://www.hamqth.com/xml.php?callsign={callsign.upper()}"
        
        headers = {
            'User-Agent': 'WFD-Logger/1.0 (Contest Logging Software)'
        }
        ...
        if search is not None:
            result = {
                'source': 'HamQTH.com',
                'callsign': callsign.upper(),
                'name': getattr(search.find('nick'), 'text', None),
                'address': getattr(search.find('adr_name'), 'text', None),
                'grid': getattr(search.find('grid'), 'text', None),
                'country': getattr(search.find('country'), 'text', None),
                'state': getattr(search.find('us_state'), 'text', None),
                'qth': getattr(search.find('qth'), 'text', None),
                'latitude': getattr(search.find('latitude'), 'text', None),
                'longitude': getattr(search.find('longitude'), 'text', None),
                'error': None
            }
...
def lookup_callsign_radiodb(callsign):
    """Lookup callsign information from Radio-DB (free amateur radio database)"""
    try:
        # Radio-DB API endpoint
        url = f"https://api.radio-db.com/callsign/{callsign.upper()}"
...
def lookup_callsign(callsign):
    """
    Main callsign lookup function - tries multiple sources
    Returns the first successful result or aggregated results
    """
    ...
    sources = [
        ...
        # lookup_callsign_qrz  # Commented out as it requires authentication
    ]
    
    results = []
    successful_result = None
    
    for lookup_func in sources:
        try:
            result = lookup_func(callsign)
            results.append(result)
            
            # Use first successful result (no error)
            if successful_result is None and not result.get('error'):
                successful_result = result
                
        except Exception as e:
            results.append({
                'source': lookup_func.__name__,
                'callsign': callsign,
    ...
    # Otherwise return aggregated error info
    return {
        'callsign': callsign,
        'error': 'No successful lookups',
        'attempts': results
    }
...
@app.route('/lookup_callsign')
def api_lookup_callsign():
